...
```{r}
d <- read.csv("./Integration/annotations_DGE.csv")
d <- filter(d, d$p_val < 0.05)
d
```

```{r}
d <- filter(d, d$cluster == "NC Mono")
```
...
cnetplot(edox, foldChange=geneList, circular = TRUE, colorEdge = TRUE) + scale_color_gradientn(colors=c("red","lightgrey", "blue"), oob=scales::squish)
```
